<template>
  <div v-if="!!user" class="min-h-screen">
    <Disclosure v-slot="{ open, close }" as="nav" class="bg-gray-800 shadow-lg">
        ...
        <div class="flex h-16 items-center justify-between">
          ...
          <div class="hidden md:block">
            <div class="ml-4 flex items-center md:ml-6">
              ...
              <!-- Profile dropdown -->
              <Menu as="div" class="relative ml-3 hover:text-white text-gray-400">
                <div>
                  <MenuButton class="px-3 flex max-w-xs items-center rounded-full bg-gray-800 text-sm">
                    <span class="sr-only">Open user menu</span>
                    <div class="flex-shrink-0">
                      <common-my-svg-icon :path="mdiCardAccountDetailsOutline" type="mdi" />
                    </div>
                    <div class="ml-3">
                ...
                >
                  ...
                  >
                    <MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active, close: clMenu }">
                      <nuxt-link
                        :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700 cursor-pointer']"
                        @click="handleClickUserNav(item,clMenu)"
                      >
                        {{ item.name }}
                      </nuxt-link>
                    </MenuItem>
      ...
      <transition
        ...
        leave-to-class="transform opacity-0 scale-95"
      >
        <DisclosurePanel class="z-10 md:hidden absolute top-[3.9rem] w-full bg-gray-800">
          <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3 ">
            <nuxt-link
              v-for="item in navigation"
              :key="item.name"
              :to="item.href"
              :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'space-x-4 relative items-center flex rounded-md px-3 py-2 text-base font-medium']"
          ...
          <div class="border-t border-gray-700 pb-3 pt-4">
            <div class="flex items-center px-5">
              <div class="flex-shrink-0">
                <common-my-svg-icon class="text-white" :path="mdiCardAccountDetailsOutline" type="mdi" />
              </div>
              <div class="ml-3">
                <div class="text-base font-medium leading-none text-white">
                  {{ fioSmall }}
    ...
    <div class="flex">
      <div
        class="z-10 hidden md:block absolute sidebar w-[3.35rem] overflow-hidden border-r hover:w-[200px] hover:bg-white hover:shadow-lg"
      >
            ...
            <ul class="space-y-2 tracking-wide">
              <template v-for="item in navigation">
                <li v-if="!item.skip" :key="item.name" class="min-w-max">
                  <nuxt-link
                    :to="item.href"
                    :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-500 hover:bg-gray-700 hover:text-white', 'space-x-4 relative items-center flex px-4 py-3 text-sm font-medium']"
                    :aria-current="item.current ? 'page' : undefined"
                  >
                    <common-my-svg-icon :path="item.icon" type="mdi" />
                    <span class="-mr-1 font-medium">{{ item.name }}</span>
...
import {
  mdiAccount,
  mdiBellOutline,
  mdiCalendarCheck, mdiCardAccountDetailsOutline,
  mdiClose,
...
import { NavigationItem, UserNavigationItem } from '~/interfaces';
import { ResponseUserDto } from '~/interfaces/apiTypes/user/dto/response-user.dto';

const route = useRoute();
const router = useRouter();
const authStore = useAuthStore();
const user = computed<ResponseUserDto | null>(() => authStore.user);
...
const handleClickUserNav = (item: UserNavigationItem, close: () => void) => {
  close();
  if (item.action) {
    item.action();
  } else if (item.href) {
